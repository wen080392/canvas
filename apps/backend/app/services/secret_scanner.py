...
class SecretScannerService:
    ...
    PATTERNS = {
        'aws_access_key': {
            'pattern': r'AKIA[0-9A-Z]{12,20}',
            'description': 'AWS Access Key ID',
            'severity': 'CRITICAL'
        },
        'aws_secret_key': {
            'pattern': r'aws_secret_access_key\s*=\s*["\']([A-Za-z0-9/+=]{35,45})["\']',
        ...
        'github_token': {
            'pattern': r'ghp_[A-Za-z0-9]{20,50}',
    ...
    def scan_github_repository(self, repo_url: str) -> List[SecretFinding]:
        """
        Scan a GitHub repository for secrets.
        
        Args:
            repo_url: Full URL of the repository (e.g., https://github.com/user/repo)
        ...
        print(f"🔍 Scanning GitHub Repository: {owner}/{repo_name}")
        
        # Get repository content (simple recursive approach for small/medium repos)
        # Note: Ideally this should clone properly, but we'll use API for simplicity
        return self._scan_github_recursive(github_service, owner, repo_name)

    def _scan_github_recursive(self, service: GitHubService, owner: str, repo_name: str, path: str = "") -> List[SecretFinding]:
        findings = []
        items = service.get_repository_content(owner, repo_name, path)
        ...
        for item in items:
            if item['type'] == 'file':
                ...
                if any(item['name'].endswith(ext) for ext in self.BINARY_EXTENSIONS) or item['name'] in self.EXCLUDE_DIRS:
                    continue

                try:
                    # Fetch file content
                    if item.get('content'):
                        # If content is base64 encoded, but API usually gives 'download_url' for raw
                        import requests
                        response = requests.get(item['download_url'])
                        content = response.text
                        
                        file_findings = self.scan_content(content, item['path'])
                        findings.extend(file_findings)
                except Exception as e:
                    print(f"Error scanning {item['path']}: {e}")
                ...
                findings.extend(self._scan_github_recursive(service, owner, repo_name, item['path']))
